(() => {
      ...
      if (position < window.innerHeight) {
        ...
        if (typeof element.dataset.animationGroup !== 'undefined') {
          ...
          inViewAnimation.forEach(ge => {
            if (
              typeof ge.dataset.animationGroup !== 'undefined' &&
              ge.dataset.animationGroup === group
            ) {
        ...
        if (typeof element.dataset.replaceTo !== 'undefined') {
          setTimeout(
            () => {
              element.dataset.toggleInView
                .split(' ')
                .forEach(animation => element.classList.remove(animation));
              element.dataset.replaceTo
                .split(' ')
                .forEach(animation => element.classList.add(animation));
            },
            typeof element.dataset.replaceIn !== 'undefined'
              ? parseInt(element.dataset.replaceIn)
              : 1300
          );
        }
      }
  ...
  function animateProposalSection() {
    let position =
      proposalSectionTitle.getBoundingClientRect().top + window.innerHeight / 3;
